...
### Added
- Support for the ShotGrid `duration` field type, mapped to SQLAlchemy `Integer` (nullable), matching other numeric types (`number`, `percent`, `timecode`).
- Regression test `test_duration_field_type` covering class generation, column type, and round-trip persistence.

## [0.3.2] - 2026-02-26

### Fixed
- Guard FK generation for single-type `entity` fields: if the referenced table is absent from the schema or is in `ignored_tables`, fall back to a plain `BigInteger` column instead of emitting `sa.ForeignKey(...)`. Prevents `NoReferencedTableError` at `metadata.create_all()` time for internal ShotGrid system entities (e.g. `AppWelcome`) that are referenced by other tables but not returned by `schema_entity_read()`.
- Added regression test `test_dangling_fk_not_generated` to cover this scenario.

## [0.3.1] - 2026-02-25

### Fixed
- Remove f-string prefix from string literal with no placeholders in `classes.py` (ruff F541)
- Sort inline `sqlalchemy` imports alphabetically inside test functions (ruff I001)
- Collapse chained `.get()` call onto a single line in `classes.py` (black formatting)
